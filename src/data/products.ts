...
// Only include the first 25 products
const imagesToInclude = originalGalleryImages.slice(0, 25).map(image => image.src);
...
export const products: Product[] = originalGalleryImages
  .filter(image => imagesToInclude.includes(image.src)) // Filter to only include the first 25
  .map((image, index) => {
